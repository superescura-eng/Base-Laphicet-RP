...
const app = new Vue({
  ...
  watch: {
    refreshRate(newD, old) {
      if (newD <= 135) {
        $.post(
          `https://${this.resourceName}/hudselected`,
          JSON.stringify({ settingstype: "refreshRate", val: 135 })
        );
      }
    },
    hideBoxData: {
  ...
  methods: {
    ...
    ResetHudPositions() {
      $.post(
        `https://${this.resourceName}/ResetHudPositions`,
        JSON.stringify({})
      );
    },
    SetShowOxy(toggle) {
    ...
    SetAllEditMode(toggle) {
      ...
      if (toggle) {
        this.ExitSettings();
        $.post(
          `https://${this.resourceName}/editModeActive`,
    ...
    SetHideShowBox(toggle) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "showHideBox", val: toggle })
      );
    },
    ...
    SetRefreshRate(e) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "refreshRate", val: e.target.value })
      );
    },
    ...
    SetSpeedometerSize() {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({
          settingstype: "speedometerSize",
          val: this.speedometerSize,
        })
      );
    },
    SetHudSize() {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "hudSize", val: this.hudSize })
      );
    },
    SetStyles(val, val2) {
    ...
    SetSelectedMap(map) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "maptype", val: map })
      );
    },
    SetShowCompass(val) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "showCompass", val })
      );
    },
    ...
    UpdateSettings(data) {
      ...
      if (data.hudColors) {
        ...
        if (prev) {
          if (
            JSON.stringify(prev) !== JSON.stringify(this.hudColors[data.hud])
          ) {
            progress = {};
    ...
    SetCurrentHud(hud) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "hud", val: hud })
      );
    },
    ...
    SetSelectSpeedUnit(val) {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "speedtype", val })
      );
    },
    ToggleHud() {
      $.post(
        `https://${this.resourceName}/hudselected`,
        JSON.stringify({ settingstype: "hide", val: !this.hide })
      );
    },
    ToggleCinematic() {
      $.post(
        `https://${this.resourceName}/toggleCinematic`,
        JSON.stringify({}),
        (val) => {
  ...
  computed: {
    ...
    GetMphButton() {
      return this.selectSpeedUnit == "mph"
        ? '<div  class="speedometerButtonActive">MPH</div>'
        : '<div @click=`SetSelectSpeedUnit(`mph`)` class="speedometerButton">MPH</div>';
    },
    GetKmhButton() {
      return this.selectSpeedUnit == "kmh"
        ? '<div class="speedometerButtonActive">KMH</div>'
        : '<div class="speedometerButton">KMH</div>';
    },

    GetCircleButton() {
      return this.selectedMap == "radial"
        ? `<div  class="circleButtonActive">${this.locales["radial"]}</div>`
        : `<div class="circleButtonDisabled">${this.locales["radial"]}</div>`;
    },
    GetRectangleButton() {
      return this.selectedMap == "rectangle"
        ? `<div class="rectangleButtonActive">${this.locales["rectangle"]}</div>`
        : `<div  class="rectangleButtonDisabled">${this.locales["rectangle"]}</div>`;
    },
    ...
    style() {
      return {
        transform:
          "translate(-50%, -95%) rotate(" + this.indicatorDegree + "deg)",
      };
    },
  },
  mounted() {
